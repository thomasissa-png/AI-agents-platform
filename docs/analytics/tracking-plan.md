<!-- Version: 2026-05-07T00:00 — @data-analyst — v2.1 alignement Phase 4 : 5 events viral/widget ajoutés (audit_share_clicked, badge_roi_clicked, roi_calculator_viewed, roi_calculator_interacted, roi_calculator_cta_clicked), pack_quota_consumed canonisé, 3 orphelins retirés des livrables (data_report_fetched, roi_calculator_calculated, onboarding_*_step_*) -->
...
- **v2.1 2026-05-07** : +5 events viral/widget (domains `viral` + `widget`) suite alignement Phase 4. Total actifs : 52. Events orphelins Phase 4 non retenus : `data_report_fetched` (V2 backlog), `roi_calculator_calculated` (privacy — inputs non loggés), `onboarding_*_step_*` (optionnel V1, hors North Star funnel).
- **Décisions clés** : (1) Naming `{domain}_{verb}_{object}` snake_case verbe au passé — conservé v1. (2) Domains v2.1 : `api`, `payment`, `audit`, `pack`, `sponsor`, `landing`, `crawl`, `cron`, `quality`, `viral`, `widget`. (3) Outil unique CF Workers Analytics Engine server-side — zéro outil tiers. (4) Volume estimé v2.1 : ~3 510-3 520 events/jour — sous quota CF AE 100 K/jour. (5) Privacy by design strict : aucun contenu audit input/output dans CF AE, aucun email, aucune IP brute, aucun UA complet. (6) `pack_quota_consumed` est le nom canonique (remplace toute occurrence de `pack_calls_used` dans les livrables Phase 4).
- **52 events actifs v2.1** : 47 actifs v2 + 5 nouveaux (viral 2 + widget 3) — 3 retirés archivés (Stripe humain pilier).
...
### 2.5b Domain `viral` — boucles de référence (NOUVEAU v2.1)

**2 events mesurant les boucles virales B2A (badge ROI et partage audit PR). Tracés côté CF Worker via redirect/serve — zéro JS client requis.**

| Event                 | Déclencheur                                                                                                   | Propriétés (typées)                                                                                                                                                                          | Outil | PII-free                                  |
| --------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ----------------------------------------- |
| `audit_share_clicked` | GET sur lien `devrefs.dev/audit/{audit_id}` depuis PR GitHub ou autre referrer externe (badge ou lien direct) | `audit_id` (string : UUID v4 pseudonyme — non corrélable wallet), `referrer_bucket` (string : github_pr / github_readme / other), `ua_bucket`                                                | CF AE | UUID pseudonyme — zéro lien wallet        |
| `badge_roi_clicked`   | GET badge SVG `devrefs.dev/badge/roi?savings=XX` — redirect vers landing page audit                           | `savings_bucket` (string : enum : low_0_15 / mid_15_30 / high_30_plus — arrondi catégoriel pour éviter fingerprint), `referrer_bucket` (string : github_readme / devto / other), `ua_bucket` | CF AE | Savings arrondi catégoriel — aucun wallet |

**Note volume v2.1** : 2-5 events/jour en V1 (boucles virales faible volume lancement). Bucket `savings_bucket` catégoriel en 3 valeurs max — ratio buckets ≤ 10 (règle privacy by design).

### 2.5c Domain `widget` — ROI Calculator embeddable (NOUVEAU v2.1)

**3 events mesurant l'engagement widget ROI Calculator (embed externe). Pas de valeurs numériques input — privacy by design strict.**

| Event                        | Déclencheur                                                                   | Propriétés (typées)                                                                                                                      | Outil                        | PII-free                                                             |
| ---------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | -------------------------------------------------------------------- |
| `roi_calculator_viewed`      | Widget ROI Calculator visible dans viewport (IntersectionObserver JS snippet) | `embed_source` (string : devrefs_landing / external_blog / external_readme), `ua_bucket`                                                 | CF AE (Worker REST endpoint) | Aucune PII — pas de valeurs slider                                   |
| `roi_calculator_interacted`  | Premier changement d'un input slider par l'utilisateur                        | `embed_source` (string), `ua_bucket`                                                                                                     | CF AE                        | Aucune PII — uniquement signal "a interagi", pas les valeurs         |
| `roi_calculator_cta_clicked` | Clic bouton CTA "Acheter Pack" dans le widget                                 | `pack_suggested` (string : discovery / standard / pro — pack recommandé par le calcul client-side), `embed_source` (string), `ua_bucket` | CF AE                        | Aucune PII — pas de montant calculé, uniquement le type pack suggéré |

**Règle privacy widget** : les valeurs numériques saisies dans les sliders (tokens/mois, coût actuel) ne sont JAMAIS transmises à CF AE. Seul le type de pack suggéré par le calcul client-side est loggué (3 valeurs enum). Cf. `roi-calculator-spec.md` § Privacy.

**Events RETIRÉS / NON RETENUS en v2.1** (orphelins Phase 4 non intégrés) :

| Event orphelin                     | Décision                                           | Justification                                                                                                                                                                            |
| ---------------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data_report_fetched`              | RETIRÉ des livrables Phase 4                       | Cron rapport JSON mensuel est V2 backlog (audit-phase4-s7.md § 7) — endpoint non prévu V1 scope                                                                                          |
| `roi_calculator_calculated`        | RETIRÉ — remplacé par `roi_calculator_interacted`  | Privacy : logguer l'état "a interagi" suffit — les valeurs numériques calculées ne doivent pas transiter vers CF AE                                                                      |
| `roi_calculator_purchased`         | RETIRÉ — remplacé par `roi_calculator_cta_clicked` | Nom non conforme convention (purchased implique transaction x402 confirmée) ; la sémantique correcte est "clic CTA"                                                                      |
| `onboarding_*_step_*` (6 variants) | RETIRÉ des livrables Phase 4                       | Optionnel V1 (handoff-sales-enablement.md § checklist) — tracking sponsor humain hors North Star funnel agent ; nécessite JS snippet dans 6 pages onboarding (effort disproportionné V1) |
...
**Compte net v2** : 38 events v1 + 12 nouveaux - 3 retirés + 2 events JWT archivés = **47 events actifs v2** (+ 3 archivés).

### Events ajoutés v2.1 (5 nouveaux — alignement Phase 4)

| Event ajouté                 | Domain | Justification                                                  |
| ---------------------------- | ------ | -------------------------------------------------------------- |
| `audit_share_clicked`        | viral  | Boucle virale 1 (badge audit PR GitHub) — signal référence     |
| `badge_roi_clicked`          | viral  | Boucle virale 2 (badge ROI embeddable) — signal référence      |
| `roi_calculator_viewed`      | widget | Funnel activation widget embed — signal intérêt sponsor        |
| `roi_calculator_interacted`  | widget | Signal engagement widget (1er input) — sans valeurs numériques |
| `roi_calculator_cta_clicked` | widget | Conversion widget → achat pack — with `pack_suggested` enum    |

**Compte net v2.1** : 47 events actifs v2 + 5 nouveaux = **52 events actifs v2.1**.
...
| `viral_*` (badge + audit share — faible volume lancement)   | 5                                    |
| `widget_*` (roi_calculator embed — faible volume lancement) | 10                                   |
| **Total estimé v2.1**                                       | **~3 515 events/jour**               |
