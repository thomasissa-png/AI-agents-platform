...
| -------------------------------------- | ------------------------------------------------------------------------------ | ----- | -------- | --------- | --------- |
| Crawls bots IA identifiés              | COUNT(crawl\__) WHERE ua_bucket LIKE 'ai_bot/_' GROUP BY day                   | CF AE | >= 10    | >= 50     | >= 300    |
...
| Ratio paiements x402 vs crawls bot IA  | COUNT(payment*x402_completed) / COUNT(crawl*_ WHERE ua_bucket LIKE 'ai_bot/_') | CF AE | >= 5 %   | >= 8 %    | >= 12 %   |
...
| Audits récurrents (même wallet_hash, 2e audit_paid_x402) | COUNT(audit_paid_x402) > 1 GROUP BY wallet_hash / COUNT(DISTINCT wallet_hash)       | CF AE | N/A M+1      | >= 10 % wallets                                                |
| Pack consumption rate (quota consommé / quota acheté)    | AVG(pack_quota_consumed.quota_pct_used) GROUP BY wallet_hash                        | CF AE | N/A M+1      | >= 60 % [HYPOTHÈSE : taux d'usage pack estimé sans données V1] |
...
| KPI                                               | Formule                                                                                          | Outil      | Cible M+1   | Cible M+6                                                              |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------- | ----------- | ---------------------------------------------------------------------- |
| Pack consumption rate (% quota consommé par pack) | AVG(pack_quota_consumed.quota_pct_used) GROUP BY pack_type                                       | CF AE + KV | >= 40 % M+1 | >= 60 % M+6 [HYPOTHÈSE]                                                |
...
| `pack_type`, `pack_quota_total`, `pack_quota_consumed` | KPIs pack (§ 3.2)                             | aucune PII                                              | 30 jours CF AE |
