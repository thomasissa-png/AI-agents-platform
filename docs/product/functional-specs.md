...
### 2.1 Tableau des 11 endpoints (F8c ajouté 2026-05-07)
...
| `/api/pack/purchase`  | POST    | x402 Coinbase facilitator (montant = prix pack) | Aucun (write KV)     | `pack_purchased`                                                                   | 400 invalid-pack-type, 402 facilitator-error, 409 pack-already-active |
...
### 2.5 Détail — `POST /api/pack/purchase` (F8c — officialisé 2026-05-07)

> **Contexte décisionnel** : endpoint officialisé suite audit-phase4-s7.md § 3.B. F8b décrivait le quota lookup KV sans définir la route d'achat. Sans cet endpoint, les packs sont invendables et l'offre Offre 1 se réduit à pay-per-call uniquement → revenu packs = $0. Décision @product-manager 2026-05-07.

**US-F8c** : En tant qu'agent IA autonome (ou sponsor humain configurant son agent), je veux appeler `POST /api/pack/purchase` avec une signature x402 pour acquérir un pack pré-payé, afin de réduire la friction des micro-paiements par call et d'accéder aux endpoints DevRefs à < 50 ms p95 pour les 10 000/60 000 calls suivants.

**Request** :

```
POST /api/pack/purchase HTTP/1.1
Host: devrefs.dev
Content-Type: application/json
X-PAYMENT: [header x402 signé Coinbase facilitator — montant = prix pack]
```
...
```json
{
  "pack": "discovery" | "standard" | "pro"
}
```

**Valeurs acceptées et prix** :

| `pack`        | Prix x402 USDC | Quota        | Prix unitaire |
| ------------- | -------------- | ------------ | ------------- |
| `"discovery"` | $5             | 5 000 calls  | $0.001        |
| `"standard"`  | $10            | 10 000 calls | $0.001        |
| `"pro"`       | $50            | 60 000 calls | $0.00083      |

**Validation avant traitement x402** :

- `pack` non fourni ou valeur hors enum → 400 `INVALID_PACK_TYPE` (avant paiement — ne pas déclencher x402)
- Pack actif non épuisé déjà présent pour ce `wallet_hash` → 409 `PACK_ALREADY_ACTIVE` + `{ calls_remaining, expires_at }` (avant paiement)

**Flux post-paiement x402 confirmé** :

1. Coinbase facilitator confirme settlement → CF Worker reçoit callback
2. Worker génère `pack_id` = `pack_{pack_type}_{wallet_hash_short}_{ts_unix}`
3. Worker écrit KV `pack:{wallet_hash}:{pack_id}` → `{ quota_remaining, quota_total, pack_type, purchased_at, expires_at (30j), endpoint_scope: ["llm-prices", "sdk-status"] }`
4. Worker émet event `pack_purchased` (CF Analytics Engine)
5. Response 200
...
{
  "pack": "standard",
  "pack_id": "pack_standard_a3f8b2_1746619200",
  "calls_remaining": 10000,
  "quota_total": 10000,
  "expires_at": "2026-06-07T00:00:00Z",
  "endpoint_scope": ["llm-prices", "sdk-status"],
  "_signature": "[HMAC-SHA256 anti-redistribution F13]"
}
```

**Response 400** (pack type invalide, avant paiement) :

```json
{
  "error": "INVALID_PACK_TYPE",
  "message": "pack must be one of: discovery, standard, pro",
  "status": 400
}
```

**Response 402** (facilitator error ou paiement insuffisant) :

```json
{
  "error": "PAYMENT_REQUIRED",
  "message": "x402 payment not confirmed by facilitator",
  "status": 402,
  "packs_available": [
    { "pack": "discovery", "price_usdc": 5 },
    { "pack": "standard", "price_usdc": 10 },
    { "pack": "pro", "price_usdc": 50 }
  ]
}
```

**Response 409** (pack actif existant) :

```json
{
  "error": "PACK_ALREADY_ACTIVE",
  "pack": "standard",
  "calls_remaining": 7823,
  "expires_at": "2026-06-01T00:00:00Z",
  "status": 409
}
```

**Latence cible** : p95 < 500 ms (settlement Coinbase facilitator inclus — dérogation vs 200 ms endpoints data, settlement on-chain est le bottleneck).

**Critères d'acceptance (Given/When/Then)** :

Happy path :

- GIVEN un agent avec wallet USDC Base >= $10 et aucun pack actif, WHEN il envoie `POST /api/pack/purchase` avec body `{"pack":"standard"}` et header x402 signé pour $10 USDC, THEN la response est HTTP 200 avec `calls_remaining: 10000` et KV `pack:{wallet_hash}:*` écrit avec `quota_remaining: 10000` et `expires_at` à J+30.
- GIVEN un agent avec un pack "discovery" actif à 0 calls restants, WHEN il envoie `POST /api/pack/purchase` avec body `{"pack":"standard"}`, THEN la response est HTTP 200 (pack précédent épuisé → nouveau pack autorisé).
- GIVEN un agent ayant acheté le pack "standard", WHEN il appelle `/api/llm-prices` sans header x402, THEN le middleware F8b lit KV et retourne HTTP 200 sans nouveau paiement x402.

Cas d'erreur :

- GIVEN n'importe quel body, WHEN le champ `pack` est absent ou vaut `"enterprise"`, THEN la response est HTTP 400 `INVALID_PACK_TYPE` — SANS déclencher de paiement x402.
- GIVEN un agent avec pack "standard" actif avec 3 000 calls restants, WHEN il envoie `POST /api/pack/purchase` avec body `{"pack":"standard"}`, THEN la response est HTTP 409 `PACK_ALREADY_ACTIVE` avec `calls_remaining: 3000` — SANS déclencher de paiement x402.

Cas limites :

- GIVEN le Coinbase facilitator répond timeout > 5 s, WHEN le CF Worker attend le settlement, THEN retourner HTTP 402 `PAYMENT_REQUIRED` avec `message: "facilitator_timeout"` — le quota KV n'est PAS écrit.
- GIVEN le body JSON est malformé (non-parseable), WHEN la requête arrive au Worker, THEN HTTP 400 `MALFORMED_BODY` avant tout traitement x402.
- GIVEN un double-POST rapide (< 500 ms, race condition client), WHEN les deux requêtes arrivent simultanément, THEN l'une obtient HTTP 200, l'autre HTTP 409 (KV write atomique CF Workers).

Permissions :

- GIVEN une requête sans header `X-PAYMENT` et sans pack actif, WHEN `POST /api/pack/purchase` est appelé, THEN HTTP 402 standard (corps x402 spec avec `packs_available[]`).

**Event analytics** :

| Event                      | Trigger                | Propriétés                                                                | Funnel                               |
| -------------------------- | ---------------------- | ------------------------------------------------------------------------- | ------------------------------------ |
| `pack_purchased`           | Response 200 confirmée | `pack_type`, `price_usdc`, `quota_total`, `wallet_hash` (anonymisé), `ts` | Revenue                              |
| `pack_purchase_failed_409` | Response 409           | `pack_type`, `existing_calls_remaining`                                   | Retention (signal pack encore actif) |
| `pack_purchase_failed_400` | Response 400           | `invalid_value_received`                                                  | Activation (erreur intégration dev)  |

**Definition of Done (@fullstack)** :

- [ ] Route `POST /api/pack/purchase` dans `src/worker/router.ts`
- [ ] Validation `pack` enum avant appel facilitateur (400 si invalide)
- [ ] Check KV `pack:{wallet_hash}:*` avant paiement (409 si actif)
- [ ] Settlement Coinbase facilitator → écriture KV atomique
- [ ] Event `pack_purchased` émis vers CF Analytics Engine
- [ ] Test unitaire : 400, 409, 200 happy path, timeout facilitator
- [ ] `tsc --noEmit` clean, lint clean, build clean avant commit

**Notes @fullstack** : implémenter dans `src/worker/routes/pack-purchase.ts` (nouveau fichier, pattern aligné sur `src/worker/routes/pack-status.ts` existant). Le KV namespace utilisé est `PACK_KV` (déjà défini §3). Pas de nouvelle dépendance npm — réutiliser le client x402 facilitator de F8. Le `wallet_hash` est extrait du header x402 settlement payload (SHA256 de l'adresse wallet).
