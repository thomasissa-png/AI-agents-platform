...
- **21 features V1 scorées RICE** (F1 + F1b + F1c + F2 à F26 + F8c hors doublons). [F8c ajouté 2026-05-07 — officialisé audit-phase4-s7.md § 3.B]
- **8 features V2 reportées** explicitement + 4 features Phase 4 post-launch.
- **Chemin critique v2** : middleware x402 unifié 3 endpoints → pack KV quota lookup (F8b) + endpoint achat pack F8c → audit endpoint → landing v2 → llms.txt 3 endpoints → dashboard.
...
| F8c — `POST /api/pack/purchase` : route d'achat pack x402 (Discovery $5 / Standard $10 / Pro $50) → crée quota KV F8b (officialisé 2026-05-07)                                                          | 5   | 5   | 0.8 | 2   | **10.00** | Sans F8c les packs sont invendables → $0 revenu packs | O3          | AJOUT 2026-05-07                        |
...
5. Middleware x402 UNIFIÉ (F8) — applique sur F1 + F2 + F1b (3 endpoints)
   ...
   ├── Endpoint achat pack POST /api/pack/purchase (F8c) — 1 signature x402 → quota KV créé → calls suivants lookup F8b
...
**Delta 2026-05-07 (F8c officialisation)** : F8c `POST /api/pack/purchase` ajouté à l'étape 5 — côte à côte avec F8b dans le même bloc middleware. Sans F8c, F8b est invendable (pas de route pour créer le quota). Effort @fullstack estimé M (< 4 h, pattern aligné sur `/api/pack/status` existant). Chemin critique inchangé — F8c s'insère dans un bloc déjà planifié.
