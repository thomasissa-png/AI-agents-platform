...
- **30 features RETENUES** (17 features épics + 4 nouvelles audit/pack/purchase + 9 support obligatoires conformité/UX). [Ajout F8c 2026-05-07 @product-manager — décision officialisation `POST /api/pack/purchase`, cf. audit-phase4-s7.md § 3.B]
...
| F8c — `POST /api/pack/purchase` : endpoint x402 d'achat pack pré-payé (Discovery $5 / Standard $10 / Pro $50 USDC). Auth : 1 signature x402 Coinbase facilitator sur le montant du pack. Response 200 : `{ pack, calls_remaining, expires_at, wallet_hash }`. Persiste quota dans KV `pack:{wallet_hash}:quota` (TTL 30j). Dépendances : F8 (middleware x402), F8b (KV quota lookup). User story : US-F8c. Effort @fullstack : M (< 4h, stack CF Workers + KV déjà en place). | 5            | 0.8        | Seule voie d'achat pack — sans cet endpoint F8b est invendable → revenu packs = $0 |
...
**Total V1** : 30 features (F1, F1b, F1c, F2-F20, F21-F26 + F8b + F8c = 30 items). Toutes implémentables sur stack Cloudflare Workers + KV.
...
5. Middleware x402 UNIFIÉ (F8) → applique sur F1 + F2 + F1b (3 endpoints)
   ...
   ├── Endpoint achat pack POST /api/pack/purchase (F8c) — 1 signature x402 → KV quota créé
   └── Watermark HMAC F13
...
**Delta vs v1** : Stripe Payment Link monte après F8 (pas prioritaire), audit endpoint F1b/F1c s'insère entre F8 et F7, middleware x402 unifié pour 3 endpoints (pas 2), pack KV lookup (F8b) dans le même bloc que F8. **Delta 2026-05-07** : F8c `POST /api/pack/purchase` officialisé (décision @product-manager, corrective audit-phase4-s7.md § 3.B) — s'insère dans le bloc F8/F8b, implémentation @fullstack estimée M (< 4h).
...
| Question                         | Réponse v2                                                                                                                                                  |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Quoi V1 ?**                    | 30 features (17 épics + 4 audit/pack/purchase + 9 support) couvrant 5 opportunités + conformité                                                             |
